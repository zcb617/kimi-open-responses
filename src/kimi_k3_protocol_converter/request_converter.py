...
def convert_request(responses_req: dict) -> dict:
    ...
    if isinstance(input_data, str):
        ...
        for m in input_data:
            ...
            if role == "assistant" and pending_reasoning_content:
                # Historical K3 responses may contain an empty message item
                # between reasoning and the function call. Keep the pending
                # reasoning until a real assistant message/tool call appears.
                if cm.get("tool_calls") or not _is_empty_content(content):
    ...
    if chat_req.get("stream") is True:
        # Kimi K3 sends the final usage chunk before data: [DONE].
    ...
    # Kimi K3 always reasons and accepts only low/high/max.
