...
def test_empty_string_content_emitted():
    """Empty string content should be emitted, not skipped."""
    converter = StreamConverter(response_id="resp-empty", model="kimi-k2.6")

    event = '{"id":"chatcmpl-1","choices":[{"delta":{"content":""}}]}'
    result = converter.process_event(event)
    assert result == '{"id":"resp-empty","output":[{"type":"output_text","text":""}]}'
...
def test_tool_call_stream():
    ...
    # Tool call events: emit once we have id + name, re-emit on each update
    assert results[0] == '{"id":"resp-tool","output":[{"type":"output_function_call","call_id":"call_1","name":"get_","arguments":""}]}'
    assert results[1] == '{"id":"resp-tool","output":[{"type":"output_function_call","call_id":"call_1","name":"get_","arguments":"{\\"ci"}]}'
